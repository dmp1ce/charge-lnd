...
class Policies:
    ...
    def match_by_node(self, channel, config):
        multiple_chans_props = ['min_channels_active','max_channels_active',
                                'min_channels_inactive','max_channels_inactive',
                                'min_capacity_active','max_capacity_active',
                                'min_capacity_inactive','max_capacity_inactive',
                                'min_ratio_active', 'max_ratio_active',
                                'min_ratio_inactive', 'max_ratio_inactive',
                                'min_ratio', 'max_ratio']
        accepted = ['id',
        ...
        if set(map(lambda n: "node." + n in config, multiple_chans_props)).issuperset({True}):
            ...
            if ('node.max_ratio_active' in config or 'node.min_ratio_active' in config):
                if active_total <= 0:
                    return False
                ...
                if (lambda s: s in config and not config.getfloat(s) >= ratio_active) \
                   ('node.max_ratio_active'): return False
                if (lambda s: s in config and not config.getfloat(s) <= ratio_active) \
                   ('node.min_ratio_active'): return False

            if ('node.max_ratio_inactive' in config or 'node.min_ratio_inactive' in config):
                ...
                if (lambda s: s in config and not config.getfloat(s) >= ratio_inactive) \
                   ('node.max_ratio_inactive'): return False
                if (lambda s: s in config and not config.getfloat(s) <= ratio_inactive) \
                   ('node.min_ratio_inactive'): return False

            if (lambda s: s in config and not config.getfloat(s) >= ratio_all) \
                ('node.max_ratio'): return False
            if (lambda s: s in config and not config.getfloat(s) <= ratio_all) \
                ('node.min_ratio'): return False

            if (lambda s: s in config and not config.getint(s) >= channels_active) \
                ('node.max_channels_active'): return False
            if (lambda s: s in config and not config.getint(s) <= channels_active) \
                ('node.min_channels_active'): return False
            if (lambda s: s in config and not config.getint(s) >= channels_inactive) \
                ('node.max_channels_inactive'): return False
            if (lambda s: s in config and not config.getint(s) <= channels_inactive) \
                ('node.min_channels_inactive'): return False

            if (lambda s: s in config and not config.getint(s) >= active_total) \
                ('node.max_capacity_active'): return False
            if (lambda s: s in config and not config.getint(s) <= active_total) \
                ('node.min_capacity_active'): return False
            if (lambda s: s in config and not config.getint(s) >= inactive_total) \
                ('node.max_capacity_inactive'): return False
            if (lambda s: s in config and not config.getint(s) <= inactive_total) \
                ('node.min_capacity_inactive'): return False
